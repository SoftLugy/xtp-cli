<template>
    <section>
        ...
        <!--列表-->
        <el-table :data="menuList" highlight-current-row v-loading="listLoading" @expand="expandHandle" style="width: 100%;">
        ...
        <el-dialog title="编辑" v-model="editFormVisible" @close="handleEditCancel">
            <el-form :model="editForm" label-width="100px" :rules="formRules" ref="editForm">
                ...
                <el-form-item label="排序值" prop="sort">
                    <el-input-number v-model="editForm.sort" placeholder="请输入排序值" :min="0" :controls="false" :max="9999" style="width: 100%"></el-input-number>
                    <!--<el-input placeholder="请输入排序值" v-model.number="editForm.sort"></el-input>-->
                </el-form-item>
        ...
        <el-dialog title="新增" v-model="addFormVisible" @close="handleAddCancel">
            <el-form :model="addForm" label-width="100x" :rules="formRules" ref="addForm">
                ...
                <el-form-item label="排序值" prop="sort">
                    <el-input-number v-model="addForm.sort" placeholder="请输入排序值" :min="0" :controls="false" :max="9999" style="width: 100%"></el-input-number>
                    <!--<el-input placeholder="请输入排序值" v-model.number="addForm.sort"></el-input>-->
                </el-form-item>
                <el-form-item label="菜单图标" prop="icon">
                    <span style="color: #1c8de0; font-size: 1.5rem"><i :class="addForm.icon"></i></span>
                    <el-button type="success" size="mini" @click.native="iconVisible = true">选择图标</el-button>
                    <span style="padding-left: 1rem">是否启用 </span><el-checkbox v-model="addForm.displayFlag"></el-checkbox>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
    ...
    export default {
        data() {
            ...
            return {
                ...
                //编辑界面数据
                editForm: {id:0,appId:'',parentId:0,url:'',code: '', name: '',icon:'',sort:0,displayFlag:true},
                ...
                //新增界面数据
                addForm: {id:0,appId:'',parentId:0,url:'',code: '', name: '',icon:'',sort:0,displayFlag:true}
            }
        },
        ...
        methods: {
            ...
            handleEditCancel(){
                ...
                this.editForm={id:0,appId:'',parentId:0,url:'',code: '', name: '',icon:'',sort:0,displayFlag:true}
            },
            handleIcon(icon){
            ...
            getMenus() {
                ...
                if(this.appList.length==0){
                    getAppList({pageIndex:1,pageSize:9999999}).then(function(result){
                        ...
                        data.map(function(item){
                            ...
                            item.url=CodeChange.specialCharacter(item.url)
                            item.code=CodeChange.specialCharacter(item.code)
                            item.name=CodeChange.specialCharacter(item.name)
            ...
            handleDel: function (index, row) {
                ...
                this.$confirm('确认删除该记录吗?', '提示', {
                 ...
                 this.listLoading = true;
                let para = { id: row.id };
                deleteMenuInfo(para).then(function(res){
                    if(res.data.code==1){
                    this.$message({
                        message: '删除成功',
                        type: 'success'
            ...
            expandHandle(row){
                return new Promise(function (resolve,reject) {
                    if(row.childs==null){
                        row.loading=true;
                        getMenuList({pageIndex:1,pageSize:999999,parentId:row.id}).then(function(res){
                            row.loading=false;
